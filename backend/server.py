...
CASES_CSV_FILE_NAME = 'sample_covid_data.csv'
...
def cases():
    if request.method == 'POST':
        ...
        x, y_total, y_new, y_active = analyzeRisk(
            client_data["rValue"], client_data["positivityRate"], data, client_data["numDays"], client_data["population"])
        return {'x': x.tolist(), 'y_total': y_total.tolist(), 'y_new': y_new.tolist(), 'y_active': y_active.tolist()}


@ app.route("/metrics", methods=['GET', 'POST'])
def metrics():
    csv_file_name = 'sample_covid_data.csv'
...
def set_cases_metrics():
    if request.method == 'POST':
        ...
        cases_df = pd.read_csv(CASES_CSV_FILE_NAME)

        cases_df.loc[[new_cases_metric["id"] - 1], [new_cases_metric["field"]]
                     ] = float(new_cases_metric["newVal"])

        cases_df.to_csv(CASES_CSV_FILE_NAME, index=False)
